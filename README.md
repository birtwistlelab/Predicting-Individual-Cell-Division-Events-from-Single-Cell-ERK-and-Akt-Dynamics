...
2. Run **ilastik** using the nuclear channel as the input images, along with the nuclei detection features mentioned in the paper. The output will generate masks of cell nuclei.
...
8.	We need to identify cell division events. Use `single_div_detectionv2.m` and followed by manual curation.

    - Manual curation. For each well, load joined_1hr_parsed data and verify cell division status. Cell division status can be verified by identifying the cell tracking ID, and looking at the mask. Looking at KTR dynamics can also assist, where true non-dividing cells are present for the entire time course compared to dividing cells.
...
2.	After completion, import the nuclear images into **ilastik**, and utilize the parameters outline in the paper. The pipeline will generate masks of the nucleus. 

3.	Load the corrected images and the masks from **ilastik** into the **CellProfiler** script `DUAL_analyst.cpproj`.
