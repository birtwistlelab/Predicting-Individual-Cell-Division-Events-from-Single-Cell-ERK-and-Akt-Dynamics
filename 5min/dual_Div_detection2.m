...
filePattern = fullfile(myFolder, 'joined_1hr*.mat');
...
for k = 1:length(matFiles)
    ...
    for i = 1:length(joined_1hr)
        extracted_data{1, i} = joined_1hr{1, i}.ERKRatioZ;
        extracted_data{2, i} = strcat(num2str(joined_1hr{1, i}.NUCTrackObjects_Label_20(1)), ',', num2str(joined_1hr{1, i}.ImageMetadata_ROI(1))); % cell name, site
    ...
    for i = 1:length(extracted_data)
        ...
        [pks, locs, w, p] = findpeaks(invertedY, 'MinPeakProminence', .8);
       if isempty(max(pks))==1;
          M(i, 1) =0;
          M(i, 2)=0;
          M(i, 3)=0;
       else
        ...
        M(i, 1) = extracted_data{1, i}(backtrack) - extracted_data{1, i}(timecourse_location);
        M(i, 2) = backtrack;
        M(i,3)=joined_1hr{1,i}.NUCTrackObjects_Label_20(1);
    ...
    % cant have a dividing cell in the first 20 tps right?
      ...
      for i=1:length(joined_1hr)
    if M(i,1)>=.24 && M(i,2)>60
        joined_1hr{3,i}=1;
        joined_1hr{4,i}=M(i,2);
    else
        joined_1hr{3,i}=0;
    end
    ...
    for i = 1:length(joined_1hr)
        ...
        if joined_1hr{3, i} == 0 && height(joined_1hr{1, i}) >= 500;
            joined_1hr{4, i} = 0;
        else
            continue
        end
