%the single reporter version of the cell division identification
%make sure you are in a folder that contains the output from ktrTablePlotter
%there's also a readme
load maxtimeBC

%% extracting files
myFolder=pwd % where are the timecourse data from ktrTablePlotter?
filePattern = fullfile(myFolder, 'joined_1hr B*.mat'); % change this to the well you want
...
for i=1:length(joined_1hr)
    extracted_data{1,i}=joined_1hr{1,i}.Ratio;
    extracted_data{2,i}=strcat(num2str(joined_1hr{1,i}.(Objects_Label)(1)),',',num2str(joined_1hr{1,i}.ImageMetadata_Site(1))); % cell name, site
...
for i =1:length(extracted_data)
    ...
    [pks,locs,w,p]= findpeaks(invertedY,'MinPeakWidth',.1); % can be adjusted to suit the timeseries
    ...
    M(i,5)=backtrack;% we backtrack to time series data before the division dip
    extracted_data{3,i}=backtrack;
end
% cant have a dividing cell in the first 20 timepoints right?
for i=1:length(joined_1hr)
...
%% SEPERATE BASED ON DIV OR NO DIV, while also trimming
...
plot(med_DIV,'linewidth',4,'Color',[0 0 .5 .8])
plot(med_NODIV,'linewidth',3,'Color',[.5 0 0 .8])
...
% you can execute the next code block if you are happy with the results
% sometimes the output is misclassified so its best to look at them
% afterwards and run div_detectionv3.m after you mod joined_1hr_parsed data

%% pulling and joining all the data together that was egf and ins treated
% myFolder=pwd;
% filePattern = fullfile(myFolder, 'joined_div C*.mat'); % label the well
% matFiles = dir(filePattern);
...
% 
% %% we should not concider b02 or any 02 its the untreated cond
% myFolder=pwd;
